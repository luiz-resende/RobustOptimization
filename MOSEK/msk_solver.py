"""
Solver to fix quadratic cone constraints. Extractesd from https://github.com/erickdelage/80624
"""
import mosek
import numpy as np
...
# Erick Delage: 13/13/2020

    for cone in form.qmat:
        index = cone[0]
        if  form.ub[index]<=0:
            tmp = form.ub[index]
            form.ub[index] = -form.lb[index]
            form.lb[index] = tmp
            form.linear[:,index]=-form.linear[:,index]
            form.obj[0][index]=-form.obj[0][index]
    return form

def solve(form, display=True, export=False):

    if isinstance(form, SOCProg):
        form = fix_quad_cone_bug(form)
